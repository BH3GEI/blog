...
```
# LeetCode #30: 串联所有单词的子串 - 动画可视化

下面是该算法的可视化动画，展示了滑动窗口的工作原理：
...
<head>
    ...
    <title>Substring with Concatenation of Words Visualization</title>
    <style>
        body {
...
<body>
    <div class="container">
        <h1>LeetCode #30: 串联所有单词的子串</h1>
        <h2>算法可视化</h2>
        
        <div class="control-panel">
            <button id="setup-btn">设置示例</button>
        ...
        <div class="visualization">
            <div>
                <h3>输入字符串</h3>
                <div id="string-container" class="string-container"></div>
            </div>
            ...
            <div class="info-panel">
                ...
                <div class="info-box">
                    <h3>当前词频表</h3>
                    <div id="current-map" class="map-container"></div>
                </div>
            </div>
    ...
    <script>
        ...
        let animationState = {
            phase: "init", // init, outer-loop, inner-loop, complete
            step: 0,
        ...
        const targetMapDiv = document.getElementById("target-map");
        ...
        autoBtn.addEventListener("click", () => {
            if (animationState.autoPlayInterval) {
                clearInterval(animationState.autoPlayInterval);
                animationState.autoPlayInterval = null;
                autoBtn.textContent = "自动播放";
            } else {
                autoBtn.textContent = "暂停";
                animationState.autoPlayInterval = setInterval(() => {
        ...
        function setup() {
            ...
            for (const word of words) {
                targetMap[word] = (targetMap[word] || 0) + 1;
            }
            ...
            animationState.phase = "outer-loop";
        ...
        function step() {
            ...
            if (animationState.phase === "outer-loop") {
                // 外层循环，处理不同的起点偏移
                if (i >= wordLen) {
                ...
                resetInnerLoop();
                animationState.phase = "inner-loop";
                animationState.step = 0;
                return;
            }
            
            if (animationState.phase === "inner-loop") {
                // 内层循环，滑动窗口处理
        ...
        // 更新字符串可视化
        function updateStringVisualization() {
            stringContainer.innerHTML = "";
            
            for (let pos = 0; pos < s.length; pos++) {
                const charDiv = document.createElement("div");
                charDiv.className = "char";
                charDiv.textContent = s[pos];
                
                if (pos >= left && pos < right && animationState.phase === "inner-loop") {
                    if (pos === left) charDiv.classList.add("left-pointer");
                    else if (pos === right - 1) charDiv.classList.add("right-pointer");
                    else {
                        // 检查这个字符是否在一个匹配的单词中
                        const wordStart = Math.floor((pos - left) / wordLen) * wordLen + left;
                        const word = s.substring(wordStart, wordStart + wordLen);
                        
                        if (targetMap[word] !== undefined) {
                            if (currentMap[word] <= targetMap[word]) {
                                charDiv.classList.add("matched-word");
                            } else {
                                charDiv.classList.add("invalid-word");
                            }
                        } else if (pos >= right - wordLen && pos < right) {
                            charDiv.classList.add(animationState.step === 2 ? "invalid-word" : "current-word");
                        }
                    }
                }
        ...
        function updateParams() {
            paramsDiv.innerHTML = `
                <p><strong>单词长度:</strong> ${wordLen}</p>
                <p><strong>单词数量:</strong> ${wordCount}</p>
                <p><strong>总长度:</strong> ${totalLen}</p>
                <p><strong>当前偏移:</strong> ${i}</p>
                <p><strong>左指针:</strong> ${left}</p>
                <p><strong>右指针:</strong> ${right}</p>
                <p><strong>匹配计数:</strong> ${count}</p>
            `;
        }
        ...
        function updateMaps() {
            // 更新目标词频表
            targetMapDiv.innerHTML = "";
            for (const word in targetMap) {
