...
func deleteDuplicates(head *ListNode) *ListNode {
    ...
    // 9. 返回虚拟头节点的 'Next' 指针，它指向修改后链表的头节点。
    return dummyHead.Next
}
...
func partition(head *ListNode, x int) *ListNode {
    //俩哑节点
    beforeDummy := &ListNode{Val:0, Next: nil}
    afterDummy := &ListNode{Val:0, Next: nil}
...
// LRU缓存的实现需要一个哈希表和一个双向链表
// 哈希表提供O(1)的查找，双向链表提供O(1)的插入和删除
type LRUCache struct {
    capacity int                   // 缓存容量
    cache    map[int]*DLinkedNode  // 哈希表用于O(1)查找
    head     *DLinkedNode          // 双向链表头节点（最近使用）
    tail     *DLinkedNode          // 双向链表尾节点（最久未使用）
}
...
func Constructor(capacity int) LRUCache {
    ...
    lruCache := LRUCache{
        ...
        head:     &DLinkedNode{0, 0, nil, nil}, // 创建哑头节点
        tail:     &DLinkedNode{0, 0, nil, nil}, // 创建哑尾节点
    }
...
func buildTree(preorder []int, inorder []int) *TreeNode {
    ...
    return build(preorder, inorder, 0, 0, len(inorder)-1, indexMap)
}

func build(preorder []int, inorder []int, preStart int, inStart int, inEnd int, indexMap map[int]int) *TreeNode {
    // 递归终止条件：中序遍历范围无效或前序遍历越界
    if inStart > inEnd || preStart >= len(preorder) {
        return nil
    }
    // 当前根节点的值（前序遍历的起始位置）
    ...
    // 递归构建左子树和右子树
    root.Left = build(preorder, inorder, preStart+1, inStart, index-1, indexMap)
    root.Right = build(preorder, inorder, preStart+leftSize+1, index+1, inEnd, indexMap)
    return root
}
...
func buildTree(inorder []int, postorder []int) *TreeNode {
    ...
    // 递归构建二叉树
    return build(postorder, inorder, len(postorder)-1, 0, len(inorder)-1, indexMap)
}

func build(postorder []int, inorder []int, postEnd int, inStart int, inEnd int, indexMap map[int]int) *TreeNode {
    // 递归终止条件：中序遍历范围无效或后序遍历越界
    ...
    // 递归构建右子树和左子树（注意顺序：先右后左）
    root.Right = build(postorder, inorder, postEnd-1, index+1, inEnd, indexMap)
    root.Left = build(postorder, inorder, postEnd-rightSize-1, inStart, index-1, indexMap)
    return root
}
```
